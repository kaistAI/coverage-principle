...
def sample_references_and_collect_ranges_multi(grouped_instances, out_json_file, n_groups=3):
    ...
    2) For each bridging group:
       ...
       c) gather up to 30 items in each of 2 example ranges:
          [-0.05..0.05], [0.90..1.00]
       (Feel free to adjust these bins.)
...
def plot_embedding(
    ...
    # 3) For coloring, let's only highlight the first m bridging groups that have >= n
...
def main():
    ...
    # Try to extract layer info from output_dir name
