...
def create_heatmaps_twohop(input_pattern, output_dir):
    ...
    for checkpoint in data.keys():
        ...
        fig_width = 2 * len(sections)  # 각 히트맵당 2.5의 너비
        fig_height = 4
        fig = plt.figure(figsize=(fig_width, fig_height))
        gs = gridspec.GridSpec(1, len(sections)+1, width_ratios=[1] * len(sections) + [0.1], wspace=0.2)
        axes = [plt.subplot(gs[i]) for i in range(len(sections)+1)]
        
        # ID Test 각 섹션에 대해 히트맵 생성
        ...
        # PDF로 저장
        output_path = os.path.join(output_dir, f"causal_tracing_detailed_grouping_{n_value}_{metric_type}_{checkpoint}.pdf")
...
def main():
    input_path = "/home/jinho/repos/GrokkedTransformer/collapse_analysis/tracing_results/2-hop/twohop.50.10000.diff-f12.inf_wd-0.1_layer-8_head-12_seed-42_extended_residual_diff_f1_prob_detailed_grouping_2_refined.json"

    output_dir_path = os.path.join("heatmaps", f"{input_path.split('/')[-1].split('_')[0]}_detailed_grouping_2")
    
    create_heatmaps_twohop(input_path, output_dir_path)
