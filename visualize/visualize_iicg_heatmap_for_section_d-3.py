                    ...
                    for layer in data[section][cutoff_or_k][f_key][step]:
                        ...
                        for pos in data[section][cutoff_or_k][f_key][step][layer]:
                            if positions is not None and pos not in positions:
                                continue
                            result[section][cutoff_or_k][f_key]["stepfinal_checkpoint"][layer][pos] = data[section][cutoff_or_k][f_key][step][layer][pos]
    return dictify(result)


def create_heatmaps_twohop(filtered_dirs_data, output_dir, vmax):
    """
    2-hop 데이터셋을 위한 ID Test와 OOD heatmap을 하나의 figure에 그립니다.
    모든 디렉토리의 히트맵을 하나의 figure에 표시합니다.
    """
    # 출력 디렉토리가 없으면 생성
    ...
    # 첫 번째 디렉토리의 데이터로 detailed_grouping 판단
    first_dir_data = filtered_dirs_data[0][1]["ID Test"]
    if "default" in first_dir_data:
        # detailed_grouping이 없는 경우
        sections = ["default"]
        section_titles = ["ID_test"]
    elif "low_cutoff" in first_dir_data and "mid_cutoff" in first_dir_data and "high_cutoff" in first_dir_data:
        n_value = 2
        sections = ["low_cutoff", "mid_cutoff", "high_cutoff"]
        section_titles = ["$k < 3$", "$k = 3$", "$k > 3$"]
    elif "low_cutoff" in first_dir_data and "high_cutoff" in first_dir_data:
        n_value = 1
        sections = ["low_cutoff", "high_cutoff"]
        section_titles = ["$k < 3$", "$k \geq 3$"]
    elif "2" in first_dir_data and "3" in first_dir_data and "4" in first_dir_data:
        n_value = 3
        sections = ["2", "3", "4"]
    ...
    available_f_keys = sorted([f for f in next(iter(first_dir_data.values())).keys() if f in next(iter(first_dir_data.values())).keys()])
    print(f"available_f_keys: {available_f_keys}")
    ...
    # 모든 step에 대해 처리
    for step in sorted(next(iter(next(iter(first_dir_data.values())).values())).keys(), key=lambda x: float('inf') if x == "stepfinal_checkpoint" else int(x.replace("step",""))):
        # 전체 figure 생성
        num_dirs = len(filtered_dirs_data)
        fig_width = 1.8 * (num_dirs * 2 + 1)  # 각 디렉토리마다 1.8의 너비 + colorbar
        fig_height = 4  # 전체 높이
        fig = plt.figure(figsize=(fig_width, fig_height))
        
        # 전체 GridSpec 생성 (각 디렉토리 + colorbar)
        outer_gs = gridspec.GridSpec(1, num_dirs + 1, width_ratios=[1] * num_dirs + [0.1], wspace=0.2)
        outer_axes = [plt.subplot(outer_gs[i]) for i in range(num_dirs + 1)]
        ...
        # 각 디렉토리에 대해 처리
        for dir_idx, (root_dir, filtered_data) in enumerate(filtered_dirs_data):
            
            # 각 디렉토리 내부의 GridSpec 생성 (ID Test와 OOD)
            inner_gs = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=outer_gs[dir_idx], wspace=0.1)
            
            # ID Test heatmap 그리기
            id_ax = plt.subplot(inner_gs[0])
            id_layer_data = filtered_data["ID Test"]["default"]["f1"][step]
            ...
            sns.heatmap(id_heatmap_data,
                        ...
                        yticklabels=[f"Layer {i}" for i in range(len(layers), 0, -1)] if dir_idx == 0 else [],  # 첫 번째 디렉토리에만 y축 레이블 표시
                        cbar=False,
                        linewidths=0.5,
                        linecolor='lightgray',
                        ax=id_ax,
            ...
            ood_layer_data = filtered_data["OOD"]["default"]["f1"][step]
            ...
            # 디렉토리 이름을 ID Test 히트맵 위에 추가
            token_set_size = f"$|\mathcal{{X}}|={root_dir.split('/')[-1].split('.')[1]}$"
            outer_axes[dir_idx].text(0.5, -0.35, token_set_size,
                        ha='center', va='center',
                        transform=outer_axes[dir_idx].transAxes,
                        fontsize=20)
        ...
        fig.add_artist(plt.Line2D([0.301, 0.301], [-0.19, 0.88], color='gray', linestyle='--', alpha=0.5, transform=fig.transFigure))
        fig.add_artist(plt.Line2D([0.491, 0.491], [-0.19, 0.88], color='gray', linestyle='--', alpha=0.5, transform=fig.transFigure))
        fig.add_artist(plt.Line2D([0.68, 0.68], [-0.19, 0.88], color='gray', linestyle='--', alpha=0.5, transform=fig.transFigure))
        
        # 전체 figure 조정
        plt.subplots_adjust(wspace=0.001)  # subplot 간의 가로 간격을 매우 좁게 설정
        plt.tight_layout(rect=[0.05, 0.05, 0.95, 1], w_pad=0.005)  # 하단 여백 추가
        
        # PDF로 저장
        if "default" in first_dir_data:
            output_path = os.path.join(output_dir, f"twohop_iicg_heatmap_step{step.replace('step','')}_combined.pdf")
        else:
            output_path = os.path.join(output_dir, f"twohop_iicg_heatmap_detailed_grouping_{n_value}_step{step.replace('step','')}_combined.pdf")
...
def create_heatmaps_nontree(filtered_dirs_data, output_dir, vmax):
    """
    ...
    # 첫 번째 디렉토리의 데이터로 f_key 목록 가져오기
    first_dir_data = filtered_dirs_data[0][1]["ID Test"]["default"]
    available_f_keys = sorted([f for f in first_dir_data.keys() if f in first_dir_data.keys()])
    num_f_keys = len(available_f_keys)
    ...
    for step in sorted(next(iter(next(iter(first_dir_data.values())).values())).keys(), key=lambda x: float('inf') if x == "stepfinal_checkpoint" else int(x.replace("step",""))):
        # 전체 figure 생성 (각 디렉토리마다 ID Test/OOD)
        num_dirs = len(filtered_dirs_data)
        fig_width = 1.8 * (num_f_keys * 2)  # 각 f_key마다 ID Test와 OOD
        fig_height = 4 * num_dirs  # 각 디렉토리마다 4의 높이
        fig = plt.figure(figsize=(fig_width, fig_height))
        gs = gridspec.GridSpec(num_dirs, num_f_keys * 2, width_ratios=[1] * (num_f_keys * 2), wspace=0.0, hspace=0.3)
        
        # 각 디렉토리에 대해 처리
        for dir_idx, (root_dir, filtered_data) in enumerate(filtered_dirs_data):
            dir_name = root_dir.split("/")[-1]
            axes = [plt.subplot(gs[dir_idx, i]) for i in range(num_f_keys * 2)]
            
            # 각 f_key에 대해 처리
            for idx, f_key in enumerate(available_f_keys):
                id_ax = axes[idx * 2]
                id_layer_data = filtered_data["ID Test"]["default"][f_key][step]
                ...
                sns.heatmap(id_heatmap_data,
                            ...
                            yticklabels=[f"Layer {i}" for i in range(len(layers), 0, -1)] if idx == 0 and dir_idx == 0 else [],  # 가장 왼쪽 heatmap에만 y축 레이블 표시
                            cbar=False,
                            linewidths=0.5,
                            linecolor='lightgray',
                            ax=id_ax,
                ...
                # y축 레이블 회전 (가로로 표시)
                if idx == 0 and dir_idx == 0:  # 첫 번째 heatmap에만 적용
                    id_ax.set_yticklabels(id_ax.get_yticklabels(), rotation=0, fontsize=16)
                ...
                # OOD heatmap 그리기
                ood_ax = axes[idx * 2 + 1]
                ood_layer_data = filtered_data["OOD"]["default"][f_key][step]
            ...
            # 디렉토리 이름을 왼쪽에 추가
            fig.text(0.02, 1 - (dir_idx + 0.5) / num_dirs, dir_name, 
        ...
        # PDF로 저장
        output_path = os.path.join(output_dir, f"nontree_iicg_heatmap_step{step.replace('step','')}_combined.pdf")
        plt.savefig(output_path, bbox_inches='tight')
        plt.close()
        print(f"Saved combined heatmap to {output_path}")
...
def main():
    ...
    parser.add_argument(
        "--root_dirs", "-r", required=True, nargs='+',
        help="similarity_metrics_layer*.txt 들이 있는 최상위 디렉토리들 (여러 개 가능)"
    )
    parser.add_argument(
    ...
    for root_dir in args.root_dirs:
        assert "results" in root_dir, f"It should be results directory: {root_dir}"

    # 데이터셋 타입 판단 (첫 번째 디렉토리 기준)
    if "2-hop" in args.root_dirs[0]:
        dataset_type = "twohop"
        collect_diffs_fn = collect_diffs_twohop
        sort_nested_fn = sort_nested_twohop
        create_heatmaps_fn = create_heatmaps_twohop
    elif "nontree" in args.root_dirs[0]:
        ...
        raise ValueError(f"Unknown dataset type in path: {args.root_dirs[0]}")
    ...
    for root_dir in args.root_dirs:
        ...
        for section in ["ID Test", "OOD"]:
            raw = collect_diffs_fn(root_dir, section)
            sorted_data = sort_nested_fn(raw)
            dir_data[section] = sorted_data
        all_dirs_data.append((root_dir, dir_data))

    # 모든 디렉토리의 데이터 필터링
    filtered_dirs_data = []
    for root_dir, dir_data in all_dirs_data:
        #########################################################
        # Should change here! 
        #########################################################
        filtered_data = filter_data(
            ...
            fs=["f1"],
            steps=["step1750", "stepfinal_checkpoint"], 
            layers=["layer1", "layer2", "layer3", "layer4", "layer5", "layer6", "layer7", "layer8"]
        )
        filtered_dirs_data.append((root_dir, filtered_data))
        
    print(filtered_dirs_data)

    # 모든 값의 최대값 찾기
    all_values = []
    for _, filtered_data in filtered_dirs_data:
    ...
    # 모든 디렉토리의 히트맵을 하나의 figure에 그리기
    output_dir_path = os.path.join(args.output_dir, "combined")
    os.makedirs(output_dir_path, exist_ok=True)

    # 모든 디렉토리의 데이터를 하나의 figure에 그리기
    create_heatmaps_fn(filtered_dirs_data, output_dir_path, vmax)
