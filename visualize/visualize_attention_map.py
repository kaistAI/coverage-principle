...
def compute_activations(model, input_strs, tokenizer, batch_size, device):
    ...
    for batch_idx in range(0, len(input_strs), batch_size):
        ...
        # print(batch_ids)
        input_ids, attention_mask = batch_ids['input_ids'], batch_ids["attention_mask"]
        ...
        for layer_idx in range(num_attention_layers):
            ...
            if batch_idx == 0:
                all_attention_maps[f"Layer_{layer_idx}"] = [attention_map_sum[0, head].cpu().numpy() for head in range(num_heads)]
                
            else:
                for i in range(num_heads):
                    all_attention_maps[f"Layer_{layer_idx}"][i] = np.add(all_attention_maps[f"Layer_{layer_idx}"][i], attention_map_sum[0, i].cpu().numpy())
    for layer_idx in range(num_attention_layers):
        for map in all_attention_maps[f"Layer_{layer_idx}"]:
            map /= len(input_strs)
...
def heatmap_for_each_input(all_attention_maps, save_dir, checkpoint):
    ...
    # Set up the matplotlib figure
    fig, axes = plt.subplots(num_attn_layer, seq_len, figsize=(2 * seq_len, 3 * num_attn_layer))
    fig.suptitle("Attention Maps Across Layers and Heads", fontsize=16)

    # Plot each attention map in the corresponding subplot
    for i, (layer, matrices) in enumerate(organized_all_attention_maps.items()):
        # Place layer title on the left side of each row with bigger font
        fig.text(0.08, 1 - (i + 1.1) / (num_attn_layer + 0.5), f"{layer}", ha='center', va='center', 
                fontsize=14, fontweight='bold', rotation=0)  # Keep rotation 0 for horizontal text
        for j, attention_matrix in enumerate(matrices):
            ...
            if i == 0:
                if j == 0:
                    ...
                    ax.set_title("<r_1>", fontweight='bold')
                elif j == 2:
                    ax.set_title("<r_2>", fontweight='bold')
            ax.axis("off")
...
def main(args):
    ...
    for checkpoint in all_checkpoints:
        ...
        with open("../data/{}/test.json".format(args.model_dir.split("/")[-1].split("_")[0])) as f:
        ...
        print(len(all_attention_maps["Layer_0"]))
        print(all_attention_maps["Layer_0"][0].shape)
