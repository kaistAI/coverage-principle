...
def main():
    ...
    # ------------------------------------------------------
    # (a) create index structure for S_f2
    S_f2_index = {}
    for (b1_idx, h3_idx), t_idx in S_f2.items():
        S_f2_index.setdefault(b1_idx, []).append((h3_idx, t_idx))

    # (b) gather expansions of the form (h1,h2)->b1, then (b1,h3)->t => 2-hop
    with mp.Pool(processes=round(mp.cpu_count() * 0.9)) as pool:
        process_S_f1 = partial(process_item_S_f1, S_f2_index=S_f2_index)
    ...
    # ------------------------------------------------------
    # We build an index for the full f2_dict for coverage classification.
    ...
    # coverage check: (h1,h2) in S_f1 => sc1; (b1,h3) in S_f2 => sc2
    for (h1_idx, h2_idx), b1_idx in f1_dict.items():
        ...
        for (h3_idx, t_idx) in f2_index.get(b1_idx, []):
            # skip stage-2
            if random.random() > (1 - skip_p):
                continue
    ...
    #    - coverage_buckets from reservoir => "type_0..3"
    # ------------------------------------------------------
    test_data = []
    ...
    save_dir = os.path.join(
        base_dir, 
        "data",
        f"twohop.{args.num_tokens}.{args.max_train_data_num}.{'same-f12' if args.same_f12 else 'diff-f12'}.inf"
    )
